...
% Empaquetado de las páginas de flotantes: cuando varias figuras o tablas caen
% juntas en una página, LaTeX las reparte con relleno vertical y deja demasiado
% hueco en blanco entre ellas. Estos ajustes alinean los flotantes en la parte
% superior y concentran el espacio sobrante al final de la página, evitando esos
% huecos. Solo afectan a la maquetación, no al contenido.
\makeatletter
\setlength{\@fptop}{0pt}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6.2cm,
      xlabel={Ronda}, ylabel={Precisión global}, xmin=0, xmax=21, ymin=0, ymax=1,
...
variante reducida del experimento e3, denominada \texttt{e3\_\allowbreak flex\_\allowbreak mini}, cuyos
...
estrategia existente (\texttt{fraction\_\allowbreak train}, \texttt{fraction\_\allowbreak evaluate},
\texttt{min\_\allowbreak available\_\allowbreak nodes}); inyectar funciones de devolución como
\texttt{evaluate\_\allowbreak fn} que el servidor ejecuta en momentos concretos; sobrescribir
métodos internos como \texttt{configure\_\allowbreak train}, \texttt{aggregate\_\allowbreak fit} o
\texttt{aggregate\_\allowbreak evaluate}; e implementar una estrategia nueva heredando de
...
progresivamente la tasa de aprendizaje leyendo \texttt{server\_\allowbreak round}— y cómo agregar
métricas con criterios propios a través de \texttt{evaluate\_\allowbreak metrics\_\allowbreak aggr\_\allowbreak fn}, en
...
(\texttt{fixedclipping\_\allowbreak mod}, \texttt{adaptiveclipping\_\allowbreak mod}) o limitar el tamaño de
los mensajes (\texttt{message\_\allowbreak size\_\allowbreak mod})~\parencite{flowerDocs}. Sobre esta
...
evidencia experimental directa, lo mismo que la reproducibilidad (F10). Los modos
de ejecución y despliegue (F6) y la integración con herramientas externas (F7)
alcanzan también la máxima puntuación, en su caso combinando evidencia experimental
y documental.
...
configuración principal alcanza un 91{,}14\,\% de precisión, muy cerca de Flower, pero con el
tiempo de ejecución más alto del estudio (algo más de tres horas) y una utilización media de GPU
elevada. El comportamiento en escalabilidad reproduce de nuevo la caída de precisión al
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccccccc}
    ...
    e1    & 5  & 20 & 0,9114 & 0,3058 & 12304 & 466 & 83,47 & 138,51 & 87 & 2,0 \\ % e1 reejecutado 25/06 con evaluacion centralizada por ronda; RAM del run nuevo pendiente de confirmar
...
En el flujo estándar, NVIDIA FLARE no evalúa el modelo global de forma centralizada en cada
ronda, sino solo al final de la ejecución. Para obtener una curva de convergencia equivalente a
la de Flower y FedML, el experimento principal se ejecutó guardando el modelo global agregado de
cada ronda; esos veinte modelos se evaluaron después de forma centralizada sobre las 10\,000
imágenes del conjunto de prueba completo. La Figura~\ref{fig:nvflare-conv} muestra esa precisión
centralizada por ronda: la convergencia es rápida en las primeras rondas y se estabiliza por
encima del 90\,\% a partir de la ronda diez, hasta una precisión final del 91{,}14\,\%. Esta es la
ejecución del experimento principal cuyas métricas se recogen en la Tabla~\ref{tab:nvflare-seq}.
  ...
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6.2cm,
      ...
      \addplot[orange, thick, mark=*] coordinates {
        (1,0.1040)(2,0.6757)(3,0.8339)(4,0.8661)(5,0.8804)(6,0.8855)(7,0.8946)
        (8,0.8997)(9,0.8970)(10,0.9036)(11,0.9028)(12,0.9039)(13,0.9091)(14,0.9098)
        (15,0.9109)(16,0.9133)(17,0.9172)(18,0.9167)(19,0.9184)(20,0.9114)};
      \addlegendentry{NVIDIA FLARE e1 (principal)}
    \end{axis}
  \end{tikzpicture}
  \caption{Convergencia de la precisión centralizada del modelo global de NVIDIA FLARE en el
  experimento principal, evaluando sobre el conjunto de prueba completo el modelo agregado de
  cada ronda.}
...
variante reducida del experimento e3, la misma \texttt{e3\_\allowbreak flex\_\allowbreak mini} empleada con
...
(\texttt{config\_\allowbreak fed\_\allowbreak server.conf} y \texttt{config\_\allowbreak fed\_\allowbreak client.conf}), en los que
...
componente se identifica con un \texttt{component\_\allowbreak id} que permite referenciarlo
...
\texttt{TFG\_\allowbreak FL\_\allowbreak STRATEGY}. En segundo lugar, FedProx se implementó en el
...
aprendizaje— cuyos argumentos incluyen \texttt{optimizer\_\allowbreak args},
\texttt{lr\_\allowbreak scheduler\_\allowbreak args}, \texttt{source\_\allowbreak model} y
...
\texttt{CUDA\_\allowbreak VISIBLE\_\allowbreak DEVICES} vacío. Por ello, el dispositivo realmente empleado
...
proximal \texttt{fedprox\_\allowbreak mu=0,001} activo durante el entrenamiento. FedAdagrad
...
CUDA, registrado como \texttt{FATAL\_\allowbreak SYSTEM\_\allowbreak ERROR}. Para aislar el problema, la
...
\texttt{fedprox\_\allowbreak mu=0,001} que aparece en la configuración no implica que FedProx se
aplique a todas las estrategias: en FedAdagrad y FedAdam el entrenamiento imprime
\texttt{fedprox\_\allowbreak mu=0,0}, porque el término proximal solo se activa cuando la
...
encapsula un trabajo federado exponiendo solo los parámetros clave —\texttt{min\_\allowbreak clients},
\texttt{num\_\allowbreak rounds}, modelo y \textit{script} de entrenamiento— y ocultando la
...
la receta como \texttt{task\_\allowbreak data\_\allowbreak filters} o \texttt{task\_\allowbreak result\_\allowbreak filters}. Soporta
...
obtiene la máxima puntuación en la personalización del entrenamiento local (F2) y en
la estrategia de agregación (F3) a partir de evidencia experimental directa, y en la
personalización del modelo y los datos (F1), los algoritmos federados disponibles
(F4), los modos de ejecución y despliegue (F6), la integración con herramientas
externas (F7) y la reproducibilidad (F10) combinando evidencia experimental y
documental. El aprendizaje asíncrono (F5) recibe puntuación parcial,
al igual que en Flower: NVIDIA FLARE lo documenta de forma explícita mediante FedBuff,
orientado al escenario de borde, con un nivel de soporte equivalente al de la
\textit{Driver API} y \textit{Flower Next} de Flower, en ambos casos respaldado solo por
la documentación y no validado en los experimentos; la diferencia de mecanismo no altera
la puntuación. La seguridad y privacidad (F8) y la
...
FedML registra la precisión de prueba centralizada del modelo global en cada ronda de
comunicación. La Figura~\ref{fig:fedml-conv} muestra esa curva para el experimento principal
(e1), la misma configuración empleada en las curvas de Flower y NVIDIA FLARE. Con ello, las tres
curvas de convergencia (Figuras~\ref{fig:flower-conv}, \ref{fig:nvflare-conv}
y~\ref{fig:fedml-conv}) corresponden ya al mismo experimento (e1) y representan la misma
magnitud —la precisión del modelo global agregado evaluado de forma centralizada sobre el
conjunto de prueba completo al final de cada ronda—, por lo que son directamente comparables
entre sí. En las tres se reproduce el mismo patrón: un ascenso muy rápido en las primeras rondas
y una estabilización posterior por encima del 90\,\%, con diferencias finales reducidas entre los
tres \textit{frameworks}.
  ...
  \begin{tikzpicture}
    \begin{axis}[width=0.9\textwidth, height=6.2cm,
      ...
      \addplot[red, thick, mark=*] coordinates {
        (1,0.2901)(2,0.7324)(3,0.8095)(4,0.8358)(5,0.8554)(6,0.8659)(7,0.8784)
        (8,0.8826)(9,0.8837)(10,0.8900)(11,0.8964)(12,0.8990)(13,0.8965)(14,0.9010)
        (15,0.9042)(16,0.9044)(17,0.9033)(18,0.9073)(19,0.9063)(20,0.9071)};
      \addlegendentry{FedML e1 (principal)}
    \end{axis}
  \end{tikzpicture}
  \caption{Convergencia de la precisión centralizada del modelo global de FedML en el experimento
  principal, evaluada sobre el conjunto de prueba completo al final de cada ronda.}
...
La configuración de FedML se apoya en un fichero \texttt{fedml\_\allowbreak config.yaml}
estructurado en secciones (\texttt{common\_\allowbreak args}, \texttt{data\_\allowbreak args},
\texttt{model\_\allowbreak args}, \texttt{train\_\allowbreak args}, \texttt{device\_\allowbreak args},
\texttt{comm\_\allowbreak args} y \texttt{tracking\_\allowbreak args}), donde se declaran los parámetros
federados como \texttt{federated\_\allowbreak optimizer}, \texttt{client\_\allowbreak num\_\allowbreak in\_\allowbreak total},
\texttt{client\_\allowbreak num\_\allowbreak per\_\allowbreak round}, \texttt{comm\_\allowbreak round}, \texttt{epochs},
\texttt{batch\_\allowbreak size}, \texttt{client\_\allowbreak optimizer} y \texttt{learning\_\allowbreak rate}~\parencite{fedmlSimApi}.
...
\texttt{federated\_\allowbreak optimizer} a \texttt{"FedAvg"} o \texttt{"FedProx"}, mientras que
FedAdam y FedAdagrad se obtienen mediante \texttt{federated\_\allowbreak optimizer:\ "FedOpt"}
...
(\texttt{fedml.run\_\allowbreak simulation()}) y una API más explícita de varias líneas que
...
\texttt{federated\_\allowbreak optimizer}~\parencite{fedmlSimOverview,fedmlSimApi}, y el módulo de
...
\texttt{enable\_\allowbreak attack} y \texttt{enable\_\allowbreak defense}~\parencite{fedmlSecurity}, además de un
...
(92{,}03\,\%), seguido de NVIDIA FLARE (91{,}14\,\%) y FedML (90{,}71\,\%). Las diferencias son
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[ybar, bar width=5pt, width=0.95\textwidth, height=6.5cm,
      ...
      \addplot coordinates {(e0,0.8260)(e1,0.9114)(e2-c4,0.8145)(e2-c5,0.8167)(e2-c8,0.7713)(e3,0.9086)};
...
\textit{frameworks} quedan en el mismo orden de magnitud temporal: FedML completa la
ejecución en 11304~s, Flower en 11864~s y NVIDIA FLARE en 12304~s, de modo que las tres
herramientas resultan muy próximas, con FedML ligeramente por delante y NVIDIA FLARE como la más
lenta en esta configuración. En los experimentos cortos de
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[ybar, bar width=5pt, width=0.95\textwidth, height=6.5cm,
      ...
      enlarge x limits=0.12, ymajorgrids]
      \addplot coordinates {(e0,5758)(e1,12304)(e2-c4,1320)(e2-c5,1404)(e2-c8,1617)(e3,5593)};
...
experimento principal (e1), FedML presenta el valor más alto (140{,}40\,Wh), seguido muy de cerca
por NVIDIA FLARE (138{,}51\,Wh) y Flower (137{,}18\,Wh), con los tres prácticamente empatados. En
participación parcial (e3),
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[ybar, bar width=7pt, width=0.95\textwidth, height=6.5cm,
      ...
      enlarge x limits=0.12, ymajorgrids]
      \addplot coordinates {(e0,51.76)(e1,138.51)(e2-c4,14.84)(e2-c5,15.08)(e2-c8,16.01)(e3,70.61)};
...
\pgfplotsset{
  metricbar/.style={
    ...
    every node near coord/.append style={font=\tiny, anchor=south,
      /pgf/number format/.cd, fixed, fixed zerofill=false, use comma, 1000 sep={}},
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \begin{tikzpicture}
      \begin{axis}[metricbar, title={Precisión}, ylabel={Acc.}, ymin=0]
        \addplot coordinates {(e0,0.8784)(e1,0.9203)(e2-c4,0.8451)(e2-c5,0.8277)(e2-c8,0.7839)(e3,0.8993)};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}\hfill
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \begin{tikzpicture}
      \begin{axis}[metricbar, title={Utilización de GPU}, ylabel={Util. (\%)}, ymin=0]
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[metricbar, title={Precisión}, ylabel={Acc.}, ymin=0]
        \addplot coordinates {(e0,0.826)(e1,0.9114)(e2-c4,0.8145)(e2-c5,0.8167)(e2-c8,0.7713)(e3,0.9086)};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[metricbar, title={Pérdida}, ylabel={Loss}, ymin=0]
        \addplot coordinates {(e0,0.5564)(e1,0.3058)(e2-c4,0.5796)(e2-c5,0.551)(e2-c8,0.6605)(e3,0.3277)};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[metricbar, title={Utilización de GPU}, ylabel={Util. (\%)}, ymin=0]
        \addplot coordinates {(e0,56.9)(e1,83.47)(e2-c4,75.97)(e2-c5,71.06)(e2-c8,61.8)(e3,87.59)};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
...
Las Figuras~\ref{fig:maxmin-gpu}, \ref{fig:maxmin-acc} y~\ref{fig:maxmin-ram} resumen,
para tres métricas representativas, el valor máximo y
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \begin{tikzpicture}
      \begin{axis}[cmpbar, title={Máximo}, ylabel={Precisión}, ymin=0, xticklabels={{{Flower\\(e1)}},{{NVFLARE\\(e1)}},{{FedML\\(e1)}}}]
        \addplot coordinates {(Flower,0.9203)(NVFLARE,0.9114)(FedML,0.9071)};
      \end{axis}
    \end{tikzpicture}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \begin{tikzpicture}
      \begin{axis}[cmpbar, title={Mínimo}, ylabel={Precisión}, ymin=0, xticklabels={{{Flower\\(e2-c8)}},{{NVFLARE\\(e2-c8)}},{{FedML\\(e2-c8)}}}]
...
92{,}03\,\%, NVIDIA FLARE 91{,}14\,\% y FedML 90{,}71\,\%), lo que confirma que, bajo
