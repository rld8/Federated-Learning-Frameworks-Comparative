...
Este capítulo presenta los resultados cuantitativos de la comparación entre Flower, NVIDIA
FLARE y FedML sobre el escenario común definido en la metodología. Primero se exponen los
resultados de cada \textit{framework} por separado y después se comparan en conjunto. Los
resultados de aprendizaje comparables proceden del modo de ejecución secuencial, según se
justifica más adelante; el modo concurrente se incorpora solo donde los datos lo permiten y,
cuando no fue viable, se documenta como una incidencia experimental.

\section{Consideraciones previas}\label{sec:consideraciones-resultados}

Conviene fijar antes algunos criterios de lectura. Cada configuración corresponde a una
ejecución con semilla fija. La precisión y la pérdida finales se refieren al modelo global
evaluado sobre el conjunto de prueba; cada \textit{framework} hace esa evaluación con su propio
mecanismo, así que las diferencias pequeñas deben interpretarse con prudencia. Se descartaron
las ejecuciones marcadas como correctas pero sin aprendizaje real (precisión estancada en torno
a 0{,}10, equivalente al azar en diez clases) y las interrumpidas antes de completar las
rondas.
...
una utilización media de GPU elevada. Los experimentos de escalabilidad muestran lo previsible:
al repartir el mismo conjunto entre más clientes, cada uno entrena con menos datos y la
precisión final desciende. El experimento de participación parcial (e3) se presenta con cautela,
porque su seguimiento por rondas quedó incompleto y su duración es bastante menor de la esperada
...
trabajo con el error \textit{OutOfMemoryError}. Las rondas recibían entonces menos resultados de
los esperados o la ejecución quedaba incompleta, con precisiones que no superaban el
comportamiento inicial. Estos resultados se tratan como una incidencia del entorno y se retoman
en la Sección~\ref{sec:concurrencia}.
...
convergencia global por ronda comparable a la de la Figura~\ref{fig:flower-conv}. Las métricas
intermedias registradas corresponden a la validación local de cada cliente.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccccc}
    ...
    \midrule
    e0    & 10 & 30 & 0,8320 & 0,4981 & 3447 & 414 MiB / 95,23\,\% / N/D & 2,6 \\
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[ybar, bar width=5pt, width=0.95\textwidth, height=6.5cm,
      ...
      ymin=0, ymax=1, ylabel={Precisión final}, legend pos=north east,
...
La Figura~\ref{fig:comp-energia} compara la energía estimada de Flower y NVIDIA FLARE; FedML
queda excluido de esta comparación porque sus métricas de GPU no son fiables salvo en el
experimento de referencia. El consumo energético sigue de cerca al tiempo de ejecución y a la
utilización de GPU: los experimentos largos con muchas épocas locales (e1 y e3) concentran el
mayor gasto. En el experimento principal ambos \textit{frameworks} consumen una energía similar
(en torno a 130--137\,Wh), mientras que NVIDIA FLARE presenta un consumo notablemente mayor en
participación parcial, asociado a su mayor tiempo de ejecución en ese caso.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[ybar, bar width=7pt, width=0.95\textwidth, height=6.5cm,
      ...
      \legend{NVIDIA FLARE, Flower}
    \end{axis}
  \end{tikzpicture}
  \caption{Energía estimada por experimento (modo secuencial) para Flower y NVIDIA FLARE.}
...
número de clientes (hasta 3727\,MiB con ocho clientes) y refleja el coste del paralelismo real
entre procesos.

\subsection{Comportamiento en modo concurrente}\label{sec:concurrencia}

El modo concurrente dejó ver las diferencias de arquitectura entre los tres \textit{frameworks}
y los límites del equipo utilizado. Cada herramienta implementa la concurrencia de una forma
distinta. Flower simula varios clientes en paralelo mediante Ray, que
...
un mayor consumo de memoria de GPU, aunque sin proporcionar una precisión final extraíble. En
conjunto, ningún \textit{framework} resultó inmune a las limitaciones de recursos en modo
concurrente, si bien solo FedML representó una concurrencia real entre procesos.
